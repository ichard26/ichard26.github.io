# Daylily's blog -- [https://ichard26.github.io/](https://ichard26.github.io/)

My personal place for writings all about FOSS and Python.

## License

Code I've written is licensed with [Apache-2.0](./LICENSE.txt). Please note that I may
include code snippets that come from other projects licensed under different terms.

Text content is licensed under [CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/).

All thoughts are my own. :)
