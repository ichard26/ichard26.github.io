...
On \<date>, we, the pip team [released pip 26.0][dpo].

This release is a pretty big one with lots of new features, including installation from
inline script metadata, excluding distributions by upload time, and in-process build
dependencies.
...
personal life that have caused me to have very little free time. I was thus unable to find
...
> This is a good time to explain why this feature is its own option. While we could've
> overloaded the `-r` option as suggested by some, `--requirements-from-script` is *not*
> equivalent to `-r`. If `requires-python` is declared in a script metadata block, pip
> will validate that the Python version is compatible as well.
...
For consistency, a timezone offset should be provided, although it is not required.
...
> While similar to uv's `--exclude-newer` option, `--uploaded-prior-to` behaves a bit differently.
> Notably, `--uploaded-prior-to` is an exclusive upper bound for both datetimes and dates. In
> other words, `--uploaded-prior-to 2025-01-01` is equivalent to `--uploaded-prior-to 00:00:00`,
> not `2025-01-01 23:59:59` as with `--exclude-newer`.

(TODO: add link to pip user guide)

We thank uv for the inspiration with their `--exclude-newer` option.
...
- Support for authentication prompts while installing build dependencies
